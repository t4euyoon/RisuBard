...
export const helpKo = {
        ...
        "chatUploadChunkSize": "일반 채팅 및 세이브 슬롯(자동 저장·퀵세이브 포함) 저장 시 한 번에 보낼 데이터 크기입니다. 기본 8MiB, 설정 범위는 1~64MiB입니다. 변경하면 다음 채팅 저장부터 앱과 서버에 함께 적용되며 재시작할 필요가 없습니다. 작게 설정할수록 요청 횟수가 늘어납니다.",
        "chatUploadChunkEnabled": "기본값은 꺼짐으로 기존 저장 방식을 유지합니다. 켜면 일반 채팅과 세이브 슬롯 모두 설정한 크기보다 큰 데이터를 나눠 업로드합니다. 끄면 채팅 전체를 한 번에 전송하므로 서버나 프록시의 요청 크기 제한에 걸릴 수 있습니다. 변경은 다음 저장부터 적용됩니다.",
